...
describe("CM Ragistration Full Test Sute", function () {
  ...
  beforeEach(() => {
    ...
    Login.fillUserPass(Cypress.env("password1")).submit({ timeout: 10000 });
  });
  ...
  it("Test Case1: Adding Blank Data", function () {
    ...
    userRegistration
      .getFirstNameError()
      .should("have.text", "The first name field is required.");
  ...
  it("Test Case2: Verify existing email ID", function () {
    cy.log("************* Adding Ragistration ****************");
    ...
    userRegistration
      .getEmailAddress({ timeout: 100000 })
      .type(this.data.EEmail, { failOnStatusCode: false });
    ...
    */
    ...
    userRegistration
      .getNotification()
      .contains("This Email Id is already registered");
  ...
  it("Test Case5: Verify Invitation send for same email ID", function () {
    ...
    userRegistration
      .getNotification()
      .contains("Invitation is already sent to this Email Id");

    cy.wait(1500);
  });

  it.only("Test Case6: Verify Roll Update", function () {
    cy.log("************* Adding Ragistration ****************");
    ...
    userRegistration.getEmailAddress({ timeout: 10000 }).type(this.data.DEmail);
    userRegistration.getFirstName({ timeout: 10000 }).type(this.data.Firstname);
    //cy.debug();
    userRegistration.getLastName({ timeout: 10000 }).type(this.data.LastName);
    
    userRegistration.getMultiSelectTag();

    //cy.get('input[placeholder="Select option"]').select(this.data.Roles).debug();
  
   // cy.get('.multiselect__input').select(this.data.Roles)
    // custom commands
    cy.roleSelect(this.data.Roles);
    
    //cy.get(':nth-child(2) > .multiselect__option > span').click()
    //cy.get('.multiselect__tags-wrap > :nth-child(1) > span').should('have.value', 'Caregiver')
    //cy.get('.multiselect__tags').should('have.value', 'Caregiver')
    /*
    
    this.data.Roles.forEach(function(element)  {
      cy.roleSelects(element)
    });
    */

    //userRegistration.getMultiSelect().should('have.value', 'Caregiver')
