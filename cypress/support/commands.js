...
Cypress.Commands.add("roleSelect", (Role) => {
  cy.get(".multiselect__element > .multiselect__option span").each(
    ($el, index, $list) => {
...
Cypress.Commands.add("roleSelects", (Roles) => {
  //cy.get('.multiselect__input').each(
  cy.get('.multiselect__content-wrapper').each(
  //cy.get(".multiselect__element > .multiselect__option span").each(
    ($el, index, $list) => {
      //if ($el.text().includes(Roles)) {
      if ($el.text()=== Roles) {
        $el.click();
...
Cypress.Commands.add("Dayrelogin", (email, password) => {
  cy.get("input[name=login_e]")
    .should("be.visible")
    .should("be.enabled")
    .type("sharad");
  cy.wait(1000);
  cy.get("input[name=login_p]")
    .should("be.visible")
    .should("be.enabled")
    .type("43214321{enter}");
  cy.wait(1000);
...
Cypress.Commands.add("SyncUPEntry", (email, password) => {
  cy.get("input[name=email]")
    .should("be.visible")
    .should("be.enabled")
    .type("sharad.samant@focalworks.in");
  cy.wait(1000);
  cy.get("input[name=password]")
    .should("be.visible")
    .should("be.enabled")
    .type("check@123{enter}");
  cy.wait(1000);
});
